...
namespace DPF {
    ...
    void ParameterHandler::DefineStringParameter(const std::string& parameter_name, const std::string& short_description, const std::string& default_value, const std::string& category_name, const std::vector<std::string>& allowed_values) {
    ...
    void ParameterHandler::CheckParameters() {
        if (GetStringParameter("file") == "") {
            std::cout << "Error: No file given!\n"; exit(1);
        }

        // TODO enable these again, when the parameters are implemented
        if (GetIntegerParameter("max-depth") > GetIntegerParameter("max-num-nodes"))
        {
            std::cout << "Error: The depth parameter is greater than the number of nodes!\n";
            exit(1);
        }

        if (GetIntegerParameter("max-num-nodes") > (uint32_t(1) << GetIntegerParameter("max-depth")) - 1)
        {
            std::cout << "Error: The number of nodes exceeds the limit imposed by the depth!\n";
            exit(1);
        }

        /*if (GetBooleanParameter("hyper-parameter-tuning") && GetStringParameter("splits-location-prefix") == "")
        {
            std::cout << "Error: hyper tuning specified but no splits given\n";
            exit(1);
        }

        if (GetBooleanParameter("hyper-parameter-tuning") && GetStringParameter("hyper-parameter-stats-file") == "")
        {
